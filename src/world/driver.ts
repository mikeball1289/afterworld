...
function move(actor: Actor, map: Map): [boolean, boolean] {
    let magnitude = Math.ceil(Math.sqrt(actor.velocity.x * actor.velocity.x + actor.velocity.y * actor.velocity.y) * 2);
        ...
        if (movingY) {
            ...
            if (actor.velocity.y < 0) {
                ...
                if (testLine({ x: actor.left, y: actor.bottom - EPSILON }, { x: actor.right - EPSILON, y: actor.bottom - EPSILON },
                    (x, y) => Map.isWalkable(map.getPixelData(x, y)), 2 ) &&
                    !testLine({ x: actor.left, y: actor.bottom - 1 - EPSILON }, { x: actor.right - EPSILON, y: actor.bottom - 1 - EPSILON }, // if we're inside of a passable solid, then we haven't actually gotten through
...
export default function update(world: World) {
    ...
    let grounded = player.velocity.y >= 0 &&
                    ...
                    (x, y) => Map.isWalkable(map.getPixelData(x, y)), 2 ) &&
                    !testLine({ x: player.left, y: player.bottom - 1 }, { x: player.right - EPSILON, y: player.bottom - 1 }, // if we're inside of a passable solid, then we haven't actually gotten through
                    (x, y) => Map.isPassable(map.getPixelData(x, y)), player.right - player.left);
